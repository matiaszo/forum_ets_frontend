...
const Topic = ({id,title, description }: {id:number, title: string; description: string }) => {
  ...
  return (
    <Link href={`${ROUTES.topic}/${id}`}>
      <div className="flex items-center shadow-md rounded-xl p-3 font-robCondensed mb-3 text-black">
        <Image className={style.image} style={{borderRadius: 4}} src={blueColor} alt="blue" />
        <div className="flex flex-col ml-10 min-w-[90%]">
          ...
          {/* <h3 className="ml-1 text-slate-500">Clique e discuta sobre esse tópico com seus colegas!</h3> */}
        </div>
      </div>
    </Link>
  );
